...
const { PostgresDatasetSystem } = require("../dataset/postgres-dataset");

const app    = express();
const engine = new RuleEngine();
const ds     = process.env.DATABASE_URL
  ? new PostgresDatasetSystem(process.env.DATABASE_URL)
  : new DatasetSystem();
...
app.post("/api/v1/diagnose", async (req, res) => {
  ...
  const datasetId = await ds.record(result.trainingData, { voltageHint: v.voltageHint, lang: v.lang });
...
app.get("/api/v1/stats", async (req, res) => res.json(ok(await ds.stats(), { version: "v1" })));
...
app.post("/api/v2/diagnose", async (req, res) => {
  ...
  const result    = engine.diagnose(v.input);
  const datasetId = await ds.record(result.trainingData, { voltageHint: v.voltageHint, lang: v.lang, source: "v2" });
  ...
  try {
    ...
    await ds.record({ input: v.input, prediction: "AI_RESPONSE", confidence: 0.9, timestamp: Date.now() }, { source: "claude_ai" });
